 ...
 * 无需复杂签名，更简单可靠
 */
public class CarConsultationUtils {

    // DashScope API配置 - 使用API Key认证
    private static final String DASHSCOPE_API_KEY = "sk-6ea7f30ab2fb45159720cf5c8b9f6a1d"; // 替换为你的API Key
...
//    }
    ...
    public static class ConsultationRecord {
        ...
        /**
     * 程序入口函数，用于演示 CarConsultationService 的使用方式。
     * 包括创建咨询请求、提交请求、等待处理结果以及查看历史记录等操作流程。
     *
     * @param args 命令行参数（本示例中未使用）
     */
    // ============== 使用示例 ==============
    public static void main(String[] args) {
        CarConsultationUtils service = new CarConsultationUtils();

        // 模拟用户ID
        long userId = 1001;

        // 创建咨询请求：预算范围18-25万，车型SUV，用途为家庭用车
        ConsultationRequest request = new ConsultationRequest(18.0, 25.0, "SUV", "家庭");
        request.setFuelPreference("混动");      // 设置燃油偏好为混动
        request.setBrandPreference("国产");     // 设置品牌偏好为国产
